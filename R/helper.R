...
#' Makes a directory for HTML charts of a CBC, if it does not already exist
#'
#' \code{MakeHtmlDirectory} will create a sub-directory if it doesn't exist yet in the ./svgs folder
#' @param sx A string
MakeHtmlDirectory <- function(sx) {
  html.dir <- paste0("html/", sx)
  cwd <- getwd()
  dir.create(file.path(cwd, html.dir), showWarnings = FALSE)
}
...
#' @param embed.file A filename for the HTML embed code
#' @param cbc.code The four-character CBC code
#' @export
PlotAreaChart <- function(cbc,
  ...
  if (nchar(embed.file)) {
    MakeHtmlDirectory(cbc.code)
    file.path = paste0('./html/', cbc.code, '/', embed.file)
    print(dfarea, 'chart', file=file.path)
  }
  else {
    plot(dfarea)
  }
